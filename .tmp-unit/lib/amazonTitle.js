...
exports.stripAmazonBranding = stripAmazonBranding;
exports.summarizeAmazonTitle = summarizeAmazonTitle;
...
const DESCRIPTOR_HINTS = /(garden|yard|porch|waterproof|auto|on\/off|birthday|gift|lover|decor|outdoor|indoor|carry on|fits under)/i;
function cleanWhitespace(input) {
    return input.replace(/\s+/g, " ").trim();
}
function stripAmazonBranding(rawTitle) {
...
function extractLeadingBrand(leftSegment) {
...
function extractTrailingBrand(value) {
...
function trimDescriptorTail(product) {
...
function trimTrailingCountPhrase(product) {
    const withoutCount = product.replace(/(?:[\s,/-]+)\d+\s*(?:count|ct|loads?|pack|packs|pcs?|pieces)\s*$/i, "");
    return cleanWhitespace(withoutCount);
}
function summarizeAmazonTitle(rawTitle) {
