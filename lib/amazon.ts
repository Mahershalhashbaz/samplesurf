...
export type AmazonLookupSource = "page" | "redirected" | "blocked";
...
export function extractAsinFromAmazonUrl(rawUrl: string): string | null {
...
function findMetaContent(
  html: string,
  matchers: Array<{ attr: "property" | "name" | "itemprop"; value: string }>,
): string | null {
...
function extractPrice(html: string): number | null {
  ...
  const offscreenPrice = html.match(
...
export async function lookupAmazonByUrl(rawUrl: string): Promise<AmazonLookupResult> {
  ...
  const decodedTitle = extractedTitle ? decodeHtmlEntities(extractedTitle) : null;
  const titleSummary = decodedTitle ? summarizeAmazonTitle(decodedTitle) : null;
